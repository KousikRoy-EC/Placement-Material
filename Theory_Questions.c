...
Here this is a list of C interview Questions that could be potentially asked in interview
...
4.  Does preprocessing happen in the case of Assembly Language Code? If "Yes" then exactly How?
5.  What is a program?
6.  Discuss C programming standardization (e.g., K&R C, C89/C90, C99, C11).
7.  What are C comments and how are they used (`//` and `/* */`)?
8.  What is the difference between declaration and definition of a variable?
9.  Explain different types of C data types, their storage sizes, and value ranges (e.g., `char`, `short`, `int`, `long`, `float`, `double`).
...
15. What is the difference between `\n` (newline) and `\r` (carriage return) escape sequences?
16. Explain about the Memory Layout of C Programs.
    *   Describe the sections of a C program's memory layout: Text segment, Initialized data segment, Uninitialized data segment (BSS), Stack, and Heap.
17. What is dynamic memory allocation in C?
    *   Explain `malloc()`, `calloc()`, `realloc()`, and `free()` functions with examples.
    *   What is the return value of `malloc(0)`?
18. What is memory leakage in ‘C’? How can it be reduced?
19. What is dynamic memory fragmentation (external and internal)?
...
25. Explain the concept of pointer data types and their purpose (how they affect operations).
26. How are pointers used in embedded programming?
27. What is a function pointer? How do you declare one? Where can function pointers be used?
...
36. Are the expressions `*ptr++` and `++*ptr` the same? Explain the difference.
...
39. Explain the `const` keyword in C.
    *   Describe properties of constant variables.
    *   When should we use `const` in a C program?
    *   Explain the meaning of various `const` declarations (e.g., `const int a;`, `int const a;`, `const int *a;`, `int * const a;`, `int const * a const;`).
    *   Discuss the placement of `const` variables in memory (local vs. global).
    *   What are the uses and advantages of `const`?
40. Explain the `volatile` keyword in C.
    *   What is the purpose and use of the `volatile` keyword?
    *   When should the `volatile` qualifier be used (memory-mapped peripheral registers, global variables modified by ISRs, global variables in multi-threaded applications)?
    *   Provide syntax examples for declaring `volatile` variables and pointers.
41. What is the difference between `const` and `volatile` qualifiers in C?
42. Can a variable be both `constant` and `volatile` in C? Provide an example of its use.
43. Explain Storage Classes in C.
    *   What are the four storage classes (`auto`, `register`, `extern`, `static`) and what do they determine (scope, visibility, lifetime)?
    *   Explain the `auto` storage class.
    *   Explain the `register` storage class.
    *   Explain the `extern` storage class.
    *   Explain the `static` storage class.
    *   Why can't `register` storage class be declared for global variables?
    *   What is the difference between global variables and static global variables?
    *   Differentiate between an internal `static` variable and an external `static` variable.
    *   Can `static` variables be declared in a header file?
    *   What are the uses of the keyword `static`?
    *   What are the different qualifiers in C (`const`, `volatile`, `restrict`)?
44. Explain "Call by Value" (Pass by Value) and "Call by Reference" (Pass by Reference) with examples.
45. What are "Inline functions" in C? What are their advantages and disadvantages?
...
49. How do you pass a function as an argument to another function (callback functions)? Provide an example.
50. What is an ISR (Interrupt Service Routine)?
51. Can you pass any parameter to or return a value from an ISR? Why/why not?
52. What is interrupt latency? How do you measure it? How can it be reduced? What are its causes?
53. What is a nested interrupt system and its rules?
54. What is NVIC (Nested Vector Interrupt Controller) in ARM Cortex?
55. Can you use any function inside an ISR? What considerations are important?
56. Can you change the interrupt priority level of Cortex-M processor family?
57. Explain "Setter" and "Getter" functions.
58. What are Macros in C?
    *   Provide examples of simple and function-like macros.
    *   Explain Conditional Macros (`#ifdef`, `#ifndef`, `#if 0`).
    *   Provide examples of Macros for Data Manipulations in C (e.g., Swap Nibbles, Swap Bytes, Swap numbers, Convert Endian).
59. Explain the differences between Macros and Functions.
60. Difference between `typedef` and `#define` (macro).
61. Difference between a macro and `const`.
62. What do you mean by enumeration (`enum`) in C?
    *   Describe the differences between Enumeration and Macro.
    *   Describe the difference between Enum and `typedef` statement.
63. Explain the difference between `#define` and `typedef` when defining pointer types for structures (e.g., `STUDENT_INFO p1, p2;` vs. `studentInfo q1, q2;`).
64. What is the difference between an Array and a Linked List?
65. What is the difference between an Array and a Structure?
66. What is the difference between a Structure and a Union in C?
67. Explain how to declare the following: a variable, an array, a pointer variable, an array of pointers, a pointer to arrays, array with functions, functions with pointer arguments, an array of structures, a pointer to structures, and a pointer within a structure.
68. Explain extracting bits of a variable using structures (Bit fields).
    *   How do you create a bit-field structure for peripheral registers?
69. What is structure padding? Is structure padding an advantage or disadvantage? If so, why and how? How can structure padding be avoided in C?
70. Discuss advanced concepts about structures, such as self-referential structures and nested structures.
71. How is `typedef` used with structures?
72. What is the concept of a `union` in C? What is its applicability in embedded system code (e.g., bit extraction, storing mutually exclusive data)?
73. Explain how to create a Singly Linked List.
    *   How to insert a node at the beginning, in the middle, and at the end of a singly linked list.
    *   How to delete a node from the beginning of a singly linked list.
74. Explain Double Linked lists for operations like: Traversing from node 1 to N, Traversing from node N to 1, Delete Operations, Insert Operations, and Copying a single linked list to another linked list.
75. Discuss important programming interview questions on arrays (e.g., finding missing/duplicate numbers, array rotation, finding elements).
76. Explain various bitwise operations in C: Set a particular bit, Clear a particular bit, Toggle a particular bit, Test a bit, Swap a nibble in a byte, and Bit Masking.
77. How do you swap odd and even numbers using bitwise operators?
78. How do you find if a given number is odd or even using bitwise operators?
79. What is the difference between Logical operators (`&&`, `||`, `!`) and Bitwise operators (`&`, `|`, `~`, `^`, `<<`, `>>`)?
80. Explain bitwise left shift (`<<`) and right shift (`>>`) operators and their common applications (e.g., multiplication/division by powers of 2).
81. Explain bit extraction using bitwise operators.
82. What is little-endian and big-endian? Provide examples.
83. Write a program to find whether the given machine is little-endian or big-endian.
84. How do you convert between little-endian and big-endian in C?
85. Write a program to check if an integer is a power of 2 using bitwise operations.
86. Write a program to count set bits in an integer (e.g., Brian Kernighan's Method).
87. Explain Decision Making in C using: `if` statement, `if-else` statement, `if-else-if` ladder, Conditional Operators (`?:`), and `switch/case` statement.
88. Explain Looping in C using: `while` loop, `for` loop, and `do-while` loop.
89. How do you code an infinite loop in C? Which way of writing infinite loops is generally more efficient than others?
90. Is a count-down-to-zero loop generally better than a count-up loop for optimization? Explain.
91. What is loop unrolling?
92. Write a program to swap two numbers without using a third variable (e.g., using arithmetic or bitwise XOR).
93. How do you print a string without using a semicolon (`;`)?
94. What is `typedef`?
95. What is conditional compilation? Explain `#if`, `#ifdef`, `#ifndef` directives.
96. How do you implement a recursive function? Provide an example (e.g., sum of natural numbers).
97. Write a program to generate a Fibonacci Series.
98. Write a program to check if a given number is a Palindrome.
99. Write a program to check if a given number is Prime.
100. Implement common string library functions (e.g., `strcpy()`, `strlen()`, `strcmp()`, `strcat()`) without using their standard library counterparts.
101. Implement mathematical functions (e.g., `sin(x)`) without using standard library functions.
102. How do you find the size of a variable without using the `sizeof` operator?
103. Explain `scanf()` and `getchar()` functions in C. What is the return value of `scanf()`?
104. What is type casting (implicit and explicit)?
105. Explain post-increment and decrement operators. Which one is generally preferred (`++i` vs `i++`) and why?
106. What is the difference between ‘C’ and ‘Embedded C’?
107. What is the start-up code in embedded systems? What actions does it perform?
108. How do you access a fixed memory location in embedded C?
109. What is the difference between RISC (Reduced Instruction Set Computer) and CISC (Complex Instruction Set Computer) processors?
110. What is the difference between I2C and SPI communication protocols? What are the limitations of the I2C interface?
111. What is the difference between Asynchronous and Synchronous Communication?
112. What is the difference between RS232 and RS485?
113. What is the difference between Bit Rate and Baud Rate?
114. What is segmentation fault in C? What are its common causes?
115. What is the difference between a Segmentation fault and a Bus error?
116. On what does the size of an integer depend? Are integers signed or unsigned by default? What is the difference between `unsigned int` and `signed int` in C?
117. Explain `stdint.h` and its importance in embedded systems programming for portability.
118. Explain the purpose and usage of the `#pragma` statement.
119. How is a program executed "Bit by bit" or "Byte by Byte?
120. What is a Watchdog Timer in embedded systems?
121. What is the function of a DMA (Direct Memory Access) Controller in Embedded Systems?
122. What is RTOS (Real-Time Operating System)?
123. Explain the boot process of a Microcontroller.
124. How are Interrupts handled by a Microcontroller?
125. Explain different methods for generating time delays using timers in microcontrollers (e.g., polling interrupt flags, tracking overflows).
126. Explain CLOCK POLARITY (CPOL) and CLOCK PHASE (CPHA) concepts related to the SPI Protocol.
127. storage class ?
compilation process ?
pointer types?
structure padding or packing?
diff b/w union and structure?
where use union or structure?(real time example)
diff b/w for loop vs while loop;
big endian vs little endian
how will you identify your machine is little or big? code
memory leak?
diff b/n calloc vs malloc?
static vs shared lib?
memory corruption ?
diff b/w function vs macro?
volatile ?
function pointer? with code
What is a Callback Function? with code
Static function?
Linear Data Structures vs Non Linear Data Structure
reverse string? normal and recursion
What happens during compilation if I use a static function?
write your own size of using macro?
find the number is odd or even inside the thread function.
string user defined function codes all
bitwise (set , clear toggel)
second highest digit
link list add ,delete, reverse, print Nth node , replace Nth node, find detect loop in link list insert
middle , delete middle
socket programming(tcp/ip vs udp)
diff b/w process vs thread?
Context switching->
types of ipc technique?
What is a system call ?
diff b/w binary semaphore vs counting semaphore?
diff b/w semaphore vs mutex?
diff b/w udp vs tcp?
Diff bw fork() vs vfork()
diff b/w mutex vs spin lock.
what is virtual memory?
what is function declaration , function definition? eg:- printf,scanf
Memory layout in C
where is initialized data and uninitialized data is stored in segments? For ex int a=10,b=20 is
stored in memory ?
What is static?
What is signed and unsigned int?
What is stack overflow ?
What is the segmentation fault?
give example for segmentation fault
difference between malloc and calloc ?
If calloc does not get contiguous memory allocation will it allocate memory or not?
What is a static variable ?
What is a void pointer?
What is memory leak?
After the dynamic allocation memory is memory freed ? will be there or free ? if it is free who
does it?
What is process and thread ?
diff between process and thread ?
what is proc
what is dev
what is difference between arm vs x86
How can we find the OS of the system?
If a binary file is given, how can we find whether it is 64 bit or 32 bit?
What is multithreading and why is it used ?
What is the condition called to stop recursive function ?Base condition
what is dangling pointer
Typecasting vs type conversion
typedef
inline vs macro
GPIO
Interrupt , interrupt vector table, Interrupt service routine
Can interrupt exist without ISR
Nested Interrupt
What are handshake signals in UART
What are different errors in CAN communication
What is interrupt latency? How to measure it?
Explain interrupts ? ISR?Interrupt vector table?
What if there are 2 interrupt waiting and 1 interrupt is in execution , then how the other 2 will be
executed?
What is device driver?
What is RTOS?
Which controller have you worked on?
What is stack and heap?
What is stack overflow?
What is a NULL pointer? Where is it used?
What is GPIO ? How to initialize GPIO ?
What is Debouncing in the controller?
What is DMA?
Which one is asynchronous communication between I2C and SPI?
Explain CAN frame ?
UART and CAN are synchronous or asynchronous communi.? What is set for communication
?(Baud rate)
What is present in the startup file?
What are different architectures available in Embedded systems?
What is a timer ? How to initialize a timer?
What is the use of makefile ?
What is a deadlock ?
Bootloader?
How the syncronisation between thread is done?
Write a code find total num of “to” in “Welcome to gdb Welcome to gdb Welcome to gdb”.
Write a code showing the pthread prototype
Write a code to count total number of set bit.
Write a function to print reverse in linked list
Interrupt handling and execution
Write a function pointer and print its output
Consider a array having pointer *p and value it has 2 3 4 5 what does *p++ and ++*p will give
as output.
What is socket programming?
1)write 2 func add, sub 2)invoke add from main using call back mechanism also pass address of
sub to add 3)invoke sub from add
Write a C code to print string reverse
